...
parts = content.split(bad_tbody)
if len(parts) == 6:
    print("Found 5 bad tables!")
    ...
    new_content = parts[0] + t1 + parts[1] + t2 + parts[2] + t3 + parts[3] + t4 + parts[4] + t5 + parts[5]
    
    with open('src/app/dashboard/page.tsx', 'w') as f:
        f.write(new_content)
    print("Fixed all tables successfully.")
else:
    print(f"Error: Found {len(parts)-1} bad tables instead of 5.")
