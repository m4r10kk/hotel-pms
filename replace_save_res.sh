#!/bin/bash
# Extract everything before saveNewReservation
awk '/async function saveNewReservation/{exit} {print}' src/app/dashboard/page.tsx > page_part1.txt

# Extract everything after saveNewReservation
awk '
  /async function saveNewReservation/ { flag = 1 }
  flag && /^  async function logout/ { flag = 0; print; next }
  !flag { print }
' src/app/dashboard/page.tsx > temp_after.txt

# Create the new function
cat << 'INNER_EOF' > new_func.txt
  async function saveNewReservation(e: React.FormEvent) {
    ...
    const selectedRoomType = roomTypes.find(rt => rt.id === selectedRoom?.room_type_id)
...
cat page_part1.txt new_func.txt temp_after.txt > src/app/dashboard/page.tsx
