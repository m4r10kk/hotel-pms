...
>
      ...
      <main id="main-content" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', paddingBottom: '80px' }}>
          ...
          {/* ══ USERS ══ */}
          {activeView === 'settings' && <SettingsView activeOrgId={activeOrgId} activeBranchId={activeBranchId} roomTypes={roomTypes} onRefreshOrg={() => loadBranches(activeOrgId)} onBranchCreated={(id) => { setActiveBranchId(id); loadBranches(activeOrgId); }} onRefresh={() => { loadRoomTypes(activeBranchId); loadRooms(activeBranchId); }} />}
          ...
          {activeView === 'users' && (
            <div className="animate-fadeUp" style={{ display: 'grid', gap: '24px' }}>
              ...
              {/* Crear Empleado */}
              <div style={{ ...card }}>
                <h2 style={{ fontSize: '15px', fontWeight: '800', marginBottom: '4px' }}>Crear Nuevo Empleado</h2>
                <p style={{ fontSize: '12px', color: 'var(--text-500)', marginBottom: '18px' }}>El empleado podrá ingresar al sistema con el correo y contraseña que le asignes aquí.</p>
                <form onSubmit={handleCreateEmployee} style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '10px', fontWeight: '800', color: 'var(--text-300)', marginBottom: '6px' }}>NOMBRE COMPLETO</label>
                    <input type="text" className="form-input" placeholder="Ej: Juan Pérez" value={empName} onChange={e => setEmpName(e.target.value)} required />
                  </div>
                  <div>
                  ...
                  <button type="submit" disabled={empLoading} style={{ background: 'var(--orange)', color: '#fff', padding: '10px 16px', borderRadius: '8px', fontSize: '12px', fontWeight: '700', border: 'none', cursor: 'pointer', opacity: empLoading ? 0.7 : 1 }}>
                  ...
                  <tbody>
                    {users.map(u => (
                      <tr key={u.id}>
                        ...
                        <td style={{ padding: '12px 14px' }}><span className="pill pill-orange">{u.system_role}</span></td>
                        <td style={{ padding: '12px 14px', color: 'var(--text-300)', fontSize: '12.5px' }}>{orgs.find(o => o.id === u.organization_id)?.name ?? '—'}</td>
                        <td style={{ padding: '12px 14px' }}><span className="pill pill-emerald">{u.is_active !== false ? 'Activo' : 'Inactivo'}</span></td>
                      </tr>
                    ))}
                    {users.length === 0 && (
                      <tr><td colSpan={4} style={{ padding: '12px 14px', color: 'var(--text-500)' }}>No hay usuarios en esta empresa</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
